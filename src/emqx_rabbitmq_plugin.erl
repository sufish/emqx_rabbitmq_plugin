...
on_client_connected(_ClientInfo = #{clientid := ClientId, username := Username}, ConnInfo, _Env) ->
  {ok, ExchangeName} = application:get_env(emqx_rabbitmq_plugin, exchange),
  ...
  Doc = {
    ...
    ipaddress, iolist_to_binary(emqx_utils:ntoa(IpAddr)),
...
on_message_publish(Message = #message{topic = Topic, flags = #{retain := Retain}}, _Env) ->
  ...
  emqx_rabbitmq_plugin_conn:publish(ExchangeName, bson_binary:put_document(Doc), <<"message.publish">>),
  {ok, Message}.
